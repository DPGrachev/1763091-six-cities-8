...
import {checkAuthAction, fetchOffersAction, fetchCurrentOfferAction, fetchFavoriteOffersAction, fetchCommentsAction, changeFavoriteStatus, fetchNearbyOffersAction, addNewCommentAction} from './api-actions';
import {APIRoute, AuthorizationStatus} from '../const';
import {State} from '../types/state';
import {requireAuthorization, setOffers, setFavoriteOffers, setCurrentOffer, setComments, setNearbyOffers, updateFavoriteOffers, updateOffers} from './action';
...
describe('Async actions', () => {
  ...
  it('should dispatch setComments when GET /comments/{room_id}', async () => {
    ...
    mockAPI
      ...
      .reply(200, comments.map((review) => adaptReviewToServer(review)));
  ...
  it('should dispatch setComments when POST /comments/{room_id}', async () => {
    const store = mockStore();
    const ROOM_ID = 1;
    const fakeComment : CommentPost = {comment : 'good', rating : 4};
    ...
    await store.dispatch(addNewCommentAction(fakeComment, ROOM_ID));
